...
export default function Applications() {
  ...
  const agencies = useMemo(
    () => [...new Set(items.map(i => i.agency?.trim()).filter(Boolean) as string[])].sort(),
    [items]
  );
  const companies = useMemo(
    () => [...new Set(items.map(i => i.company?.trim()).filter(Boolean) as string[])].sort(),
    [items]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if (filters.dateFrom) r = r.filter(i => i.applied_at >= filters.dateFrom!);
    if (filters.dateTo) r = r.filter(i => i.applied_at <= filters.dateTo!);
    if (filters.agency) r = r.filter(i => (i.agency ?? "").toLowerCase() === filters.agency!.toLowerCase());
    if (filters.company) r = r.filter(i => (i.company ?? "").toLowerCase() === filters.company!.toLowerCase());
    if (filters.salaryMin) {
      const m = Number(filters.salaryMin);
      r = r.filter(i => (i.salary_amount ?? 0) >= m);
    }
    if (filters.salaryMax) {
      const m = Number(filters.salaryMax);
      r = r.filter(i => (i.salary_amount ?? 0) <= m);
    }
    // Default ordering: most recent first (already from DB), keep it
    return r;
  }, [items, filter, q, filters]);
        ...
        <div className="flex gap-2">
          ...
          <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
            ...
            <SheetContent side="bottom" className="rounded-t-3xl max-h-[85vh] overflow-y-auto">
              <SheetHeader>
                <SheetTitle className="font-serif text-2xl">Filtri avanzati</SheetTitle>
              </SheetHeader>
              <div className="space-y-4 mt-4 pb-6">
                <div className="grid grid-cols-2 gap-3">
                  <FilterField label="Data dal">
                    <Input type="date" value={filters.dateFrom ?? ""}
                      onChange={(e) => setFilters(p => ({ ...p, dateFrom: e.target.value || undefined }))}
                      className="rounded-xl" />
                  </FilterField>
                  <FilterField label="Data al">
                    <Input type="date" value={filters.dateTo ?? ""}
                      onChange={(e) => setFilters(p => ({ ...p, dateTo: e.target.value || undefined }))}
                      className="rounded-xl" />
                  </FilterField>
                </div>
                <FilterField label="Agenzia">
                  <select
                    value={filters.agency ?? ""}
                    onChange={(e) => setFilters(p => ({ ...p, agency: e.target.value || undefined }))}
                ...
                <FilterField label="Azienda">
                  <select
                    value={filters.company ?? ""}
                    onChange={(e) => setFilters(p => ({ ...p, company: e.target.value || undefined }))}
                    className="w-full h-10 px-3 rounded-xl border border-input bg-background text-sm"
                  >
                    <option value="">Tutte</option>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <FilterField label="Retribuzione min €">
                    <Input type="number" value={filters.salaryMin ?? ""}
                      onChange={(e) => setFilters(p => ({ ...p, salaryMin: e.target.value || undefined }))}
                      className="rounded-xl" placeholder="0" />
                  </FilterField>
                  <FilterField label="Retribuzione max €">
                    <Input type="number" value={filters.salaryMax ?? ""}
                      onChange={(e) => setFilters(p => ({ ...p, salaryMax: e.target.value || undefined }))}
                      className="rounded-xl" placeholder="∞" />
                  </FilterField>
                </div>
                <div className="flex gap-2 pt-2">
                  <Button variant="outline" className="flex-1 rounded-xl" onClick={() => setFilters({})}>
                    Azzera
                  </Button>
                ...
                <div className="flex gap-3 items-start">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="rounded-xl">
                      ...
                      <DropdownMenuItem onClick={() => setPendingDelete(a)} className="text-destructive focus:text-destructive focus:bg-destructive/10">
                        Elimina candidatura
                      </DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
        ...
        <AlertDialogContent className="rounded-2xl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {pendingDelete && <>Stai per eliminare <strong>{pendingDelete.company || pendingDelete.agency}</strong> — {pendingDelete.role}. L'azione è permanente.</>}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>

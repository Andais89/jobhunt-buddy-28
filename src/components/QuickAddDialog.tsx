        ...
        <div className="space-y-4 mt-2">
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <Input id="link" value={link} onChange={(e) => setLink(e.target.value)} placeholder="https://..." className="rounded-none" />
              <Button type="button" variant="outline" onClick={importFromLink} disabled={importing || !link.trim()} className="rounded-none shrink-0">
          ...
          <div className="space-y-2">
            ...
            <Input id="company" value={company} onChange={(e) => setCompany(e.target.value)} className="rounded-none" autoFocus />
          </div>

          <div className="space-y-2">
            <Label htmlFor="role" className="text-[10px] uppercase tracking-editorial">Ruolo *</Label>
            <Input id="role" value={role} onChange={(e) => setRole(e.target.value)} className="rounded-none" />
          </div>
          ...
          <Button onClick={save} disabled={saving} className="w-full rounded-none">

    ...
    <div className="min-h-dvh bg-background flex justify-center">
      <div className="w-full max-w-[480px] min-h-dvh flex flex-col bg-paper border-x border-linen relative">
        ...
        <button
          ...
          className="fixed bottom-24 right-1/2 translate-x-[200px] sm:translate-x-[210px] z-30 size-14 bg-foreground text-background flex items-center justify-center shadow-paper hover:bg-accent transition-colors"
        >
          <Plus className="h-6 w-6" strokeWidth={1.5} />
        </button>

        {/* Bottom Nav */}
        <nav className="fixed bottom-0 w-full max-w-[480px] bg-paper border-t border-linen z-20 safe-bottom">
          <div className="grid grid-cols-5 px-2 pt-3 pb-2">
              ...
              >
                {({ isActive }) => (
                  <>
                    ...
                    <div className={cn("h-0.5 w-1 rounded-full mt-0.5", isActive ? "bg-foreground" : "bg-transparent")} />
                  </>
                )}
              </NavLink>
            ))}
